  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-[60px] z-30 glass border-b border-slate-200 dark:border-white/5 flex items-center lg:pl-[240px]">
      <div className="flex items-center gap-3 px-4 w-full">
        ...
        <img src="/india_matrix_logo.png?v=3" alt="India Matrix Logo" className="h-7 w-auto object-cover rounded-md lg:hidden" />

        {/* Current PM Badge */}
        <div 
          className={`hidden sm:flex items-center gap-2 glass rounded-lg px-3 py-1.5 transition-all ${pmEraMode ? 'border-purple-500 shadow-[0_0_15px_rgba(168,85,247,0.3)]' : ''}`}
          style={{ borderColor: pmEraMode ? '#A855F7' : `${pmColor}30` }}
        >
          <div className="w-2 h-2 rounded-full animate-pulse" style={{ background: pmColor }} />
          <span className="text-sm text-slate-500 dark:text-white/60">Era:</span>
          <select 
          ...
          >
            ...
            {PRIME_MINISTERS.map(pm => (
              <option key={pm.id} value={pm.id} className="bg-white text-slate-900 dark:bg-[#070c1a] dark:text-white">
                {pm.name.split('(')[0].trim()} ({pm.start_year}-{pm.end_year === 2026 ? 'Present' : pm.end_year})
              </option>
            ))}
          </select>
          {!pmEraMode && (
