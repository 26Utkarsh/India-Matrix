...
const ISRO_DATA = [
  { year: 1947, budget_cr: 0, satellites: 0, missions: 0, milestone: 'Pre-space era (No space agency)' },
  { year: 1962, budget_cr: 0.1, satellites: 0, missions: 0, milestone: 'INCOSPAR established by Dr. Vikram Sarabhai' },
  { year: 1963, budget_cr: 0.2, satellites: 0, missions: 0, milestone: 'First sounding rocket launch from Thumba' },
  { year: 1969, budget_cr: 1, satellites: 0, missions: 0, milestone: 'ISRO officially founded' },
  { year: 1975, budget_cr: 10, satellites: 1, missions: 1, milestone: 'Aryabhata launched' }, // Aryabhata
  { year: 1980, budget_cr: 50, satellites: 3, missions: 2, milestone: 'SLV-3 orbit success' }, // SLV-3
...
export default function SpacePage() {
  ...
  if (pmEraMode && activePMId) {
    ...
    if (pm) {
      data = ISRO_DATA.filter(d => d.year <= pm.end_year);
    }
  } else {
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="glass rounded-2xl p-6 border-l-4 border-l-purple-500 relative overflow-hidden flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
