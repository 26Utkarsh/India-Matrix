...
import { Sliders, Sparkles, TrendingUp, Zap, GraduationCap, Wifi, Share2 } from 'lucide-react';
...
export default function SimulatorPage() {
  ...
  useEffect(() => {
    ...
    if (infra) setInfraCapex(parseFloat(infra));
    if (solar) setSolarGw(parseInt(solar));
    if (edu) setEduSpend(parseFloat(edu));
    if (digital) setDigitalPush(parseInt(digital));
  }, []);

  const handleShare = () => {
    const shareUrl = `${window.location.origin}${window.location.pathname}?infra=${infraCapex}&solar=${solarGw}&edu=${eduSpend}&digital=${digitalPush}`;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass rounded-2xl p-6 border border-slate-200 dark:border-white/10 space-y-6 lg:col-span-1">
          ...
          <button 
            onClick={handleShare}
            className="w-full mt-4 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-extrabold bg-sky-500 hover:bg-sky-600 text-white shadow-md transition-all active:scale-98 cursor-pointer"
          >
            <Share2 size={13} />
            {copied ? "✓ Copied shareable URL!" : "Share Your Policy Vision"}
          </button>
        </div>
