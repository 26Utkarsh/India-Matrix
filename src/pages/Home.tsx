...
import { TrendingUp, TrendingDown, Activity, Zap, GraduationCap,
  Heart, Wifi, Train, Map, Globe2, Rocket, ArrowUpRight, ChevronLeft, ChevronRight } from 'lucide-react';
...
const LANDMARKS = [
  {
    name: "Taj Mahal, Agra",
    desc: "Iconic Symbol of India's Heritage",
  ...
  },
  {
    ...
    desc: "Majestic Seat of the Wodeyar Dynasty",
    image: "https://images.unsplash.com/photo-1598977123418-45f04b615d0e?q=80&w=600&auto=format&fit=crop"
  }
];
...
const MetricCard: React.FC<MetricCardProps> = ({ label, value, change, positive, icon, color, subtext, sparklineData }) => (
  <motion.div
    className="glass rounded-xl p-4 border transition-all cursor-default select-none relative overflow-hidden flex flex-col justify-between h-[135px]"
  ...
  >
    <div>
      ...
      <div className="metric-number text-lg font-bold text-slate-900 dark:text-white mt-0.5 leading-none">
        <AnimatedCount value={value} />
      </div>
      <div className="text-[11px] font-medium text-slate-500 dark:text-white/40 mt-1">{label}</div>
    </div>
    
    <div className="flex items-end justify-between mt-2 pt-2 border-t border-slate-100 dark:border-white/5 gap-2">
      <div className="text-[10px] text-slate-400 dark:text-white/20 font-mono truncate max-w-[60%]">{subtext}</div>
      {sparklineData && (
...
export default function HomePage() {
  ...
  // Auto rotate landmarks
  useEffect(() => {
    const timer = setInterval(() => {
      setLandmarkIdx(prev => (prev + 1) % LANDMARKS.length);
    }, 7000);
    return () => clearInterval(timer);
  }, []);

  // Sparkline history datasets matching timeline slider position
  const sparklineData = useMemo(() => {
    const history = NATIONAL_METRICS.filter(d => d.year <= currentYear);
    return {
      gdp: history.map(d => d.gdp_usd_billion),
      pop: history.map(d => d.population_million),
      lit: history.map(d => d.literacy_pct),
      life: history.map(d => d.life_expectancy),
      power: history.map(d => d.power_capacity_gw),
      internet: history.map(d => d.internet_penetration_pct),
      railway: history.map(d => d.railway_km),
      exports: history.map(d => d.exports_usd_billion),
      highways: history.map(d => d.highway_km),
      infant: history.map(d => d.infant_mortality),
    };
  }, [currentYear]);
  ...
  return (
    <div className="space-y-6">
      {/* Hero Banner */}
      <div className="relative rounded-2xl overflow-hidden glass border border-slate-200 dark:border-white/5 p-6 lg:p-8 flex flex-col md:flex-row items-center justify-between gap-6 indian-art-mandala">
        ...
        <div className="flex flex-col gap-4 items-stretch w-full md:w-[320px] lg:w-[350px] shrink-0 z-10">
          {/* Landmark Image Frame (Large & Majestic Carousel) */}
          <div className="relative w-full h-[180px] lg:h-[210px] rounded-xl overflow-hidden border border-slate-200 dark:border-white/10 shadow-md group/carousel">
            <AnimatePresence mode="wait">
              <motion.img 
                key={landmarkIdx}
                src={LANDMARKS[landmarkIdx].image} 
                alt={LANDMARKS[landmarkIdx].name}
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex items-end p-3">
              <div className="text-white w-full pr-12">
                <span className="text-xs font-bold tracking-wide block">{LANDMARKS[landmarkIdx].name}</span>
                <span className="text-[10px] opacity-75 font-mono leading-tight block">{LANDMARKS[landmarkIdx].desc}</span>
              </div>
            </div>
            {/* Carousel navigation buttons */}
            <div className="absolute right-2 bottom-2 flex gap-1 z-20 opacity-0 group-hover/carousel:opacity-100 transition-opacity">
              <button 
                onClick={() => setLandmarkIdx(prev => (prev - 1 + LANDMARKS.length) % LANDMARKS.length)}
                className="w-6 h-6 rounded-full bg-black/60 hover:bg-black/80 text-white flex items-center justify-center border border-white/10 transition-transform active:scale-90"
              >
                <ChevronLeft size={12} />
              </button>
              <button 
                onClick={() => setLandmarkIdx(prev => (prev + 1) % LANDMARKS.length)}
                className="w-6 h-6 rounded-full bg-black/60 hover:bg-black/80 text-white flex items-center justify-center border border-white/10 transition-transform active:scale-90"
              >
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          <MetricCard label="GDP" value={fmtB(metric.gdp_usd_billion)} icon={<TrendingUp size={16} />} color="#00FF88"
            ...
            sparklineData={sparklineData.gdp} />
          <MetricCard label="Population" value={`${fmt(metric.population_million, 0)}M`} icon={<Globe2 size={16} />} color="#00D4FF"
            ...
            sparklineData={sparklineData.pop} />
          <MetricCard label="Literacy Rate" value={`${fmt(metric.literacy_pct, 0)}%`} icon={<GraduationCap size={16} />} color="#9333EA"
            ...
            sparklineData={sparklineData.lit} />
          <MetricCard label="Life Expectancy" value={`${fmt(metric.life_expectancy, 1)} yrs`} icon={<Heart size={16} />} color="#F43F5E"
            ...
            sparklineData={sparklineData.life} />
          <MetricCard label="Power Capacity" value={`${fmt(metric.power_capacity_gw, 0)} GW`} icon={<Zap size={16} />} color="#FFD700"
            ...
            sparklineData={sparklineData.power} />
          ...
          <MetricCard label="Exports" value={fmtB(metric.exports_usd_billion)} icon={<ArrowUpRight size={16} />} color="#4ADE80"
            ...
            sparklineData={sparklineData.exports} />
          <MetricCard label="Highways" value={`${(metric.highway_km/1000).toFixed(0)}K km`} icon={<Map size={16} />} color="#A78BFA"
            ...
            sparklineData={sparklineData.highways} />
          <MetricCard label="Infant Mortality" value={`${fmt(metric.infant_mortality, 0)}`} icon={<Heart size={16} />} color="#F97316"
            ...
            sparklineData={sparklineData.infant} />
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-2">
          {[
            ...
            >
              <Link
                to={path}
                className="glass rounded-xl p-3 flex flex-col items-center justify-center gap-1.5 text-center group w-full h-full border hover:border-slate-300 dark:hover:border-white/10 transition-colors"
                style={{ borderColor: `${color}15` }}
              >
                <span className="text-xl group-hover:scale-110 transition-transform duration-200">{icon}</span>
                <span className="text-[11px] font-semibold text-slate-600 dark:text-white/50 group-hover:text-slate-900 dark:group-hover:text-white/80 transition-colors leading-tight">{label}</span>
              </Link>
            </motion.div>
          ))}
        </div>
