...
export default function GeopoliticalPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="glass rounded-2xl p-6 border-l-4 border-rose-600 dark:border-rose-500 flex flex-row items-center justify-between gap-4">
      ...
      <div className="flex flex-wrap gap-2">
        {types.map(t => (
          <button
            key={t}
            onClick={() => setFilter(t)}
            className={`flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-lg border transition-all font-medium capitalize
              ${filter === t ? '' : 'glass border-slate-200 dark:border-white/10 text-slate-500 hover:text-slate-800 dark:text-white/40 dark:hover:text-white/70'}`}
